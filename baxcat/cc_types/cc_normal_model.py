...
class cc_normal(object):
    ...
    def __init__(self, N=0, sum_x=0, sum_x_sq=0, m=0, r=1, s=1, nu=1, distargs=None):
        """
        Optional arguments:
    ...
    def singleton_logp(self, x):
        ...
        return self.calc_marginal_logp(1, x, x*x, self.m, self.r, self.s, self.nu)
    ...
    def calc_predictive_logp(x, N, sum_x, sum_x_sq, m, r, s, nu):
        ...
        rm, num, mm, sm = cc_normal.posterior_update_parameters(
            N+1, sum_x+x, sum_x_sq+x*x, m, r, s, nu)

        ZN = cc_normal.calc_log_Z(rn, sn, nun)
        ZM = cc_normal.calc_log_Z(rm, sm, num)
    ...
    def calc_marginal_logp(N, sum_x, sum_x_sq, m, r, s, nu):
        ...
        Z0 = cc_normal.calc_log_Z(r, s, nu)
        ZN = cc_normal.calc_log_Z(rn, sn, nun)
    ...
    def calc_log_Z(r, s, nu):
        Z = ((nu+1.0)/2.0)*LOG2 + .5*LOGPI - .5*log(r) - (nu/2.0)*log(s) + gammaln(nu/2.0)
        return Z
    ...
    def calc_m_conditional_logps(clusters, m_grid, r, s, nu):
        lps = []
        for m in m_grid:
            lp = cc_normal.calc_full_marginal_conditional(clusters, m, r, s, nu)
            lps.append(lp)
    ...
    def calc_full_marginal_conditional(clusters, m, r, s, nu):
        ...
        for cluster in clusters: 
            ...
            l = cc_normal.calc_marginal_logp(N, sum_x, sum_x_sq, m, r, s, nu)
            lp += l
    ...
    def plot_dist(X, clusters, distargs=None):
        ...
        for k in range(K):
            ...
            for n in range(200):
                y = Y[n]
                pdf[k, n] = numpy.exp(w + cc_normal.calc_predictive_logp(y, N, sum_x,
                            sum_x_sq, m, r, s, nu))
